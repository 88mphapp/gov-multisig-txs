# brownie run scripts/aug-24-2022.py --network mainnet-fork
...
def main():
    ...
    mph.transfer(gauge_rewards_distributor.address, 12000 * 1e18) # 12 weeks of rewards, 1k MPH per week

    ######################################################################
    # Update global emissions rate
    ######################################################################

    week = 7 * 24 * 60 * 60
    gauge_rewards_distributor.setGlobalEmissionRate(1000 * 1e18 / week) # 1k MPH per week
    ...
    # enable the gov multisig to distribute rewards
    vesting_contract.setRewardDistributor(
        "0x56f34826Cc63151f74FA8f701E4f73C5EAae52AD", True)
